...
(cl-defun pgmacs--row-as-json (&rest _ignore)
  ...
  (cl-labels ((jsonable-p (val)
                (cl-typecase val
                  ...
                  (hash-table t)
                  (t nil))))
    (let* ((tbl (or (pgmacstbl-current-table)
                    (user-error "No PGmacs table in this buffer")))
...
has a primary key."
  ...
  (let* ((con pgmacs--con)
         ...
         (table pgmacs--table)
         (pgmacstbl (or (pgmacstbl-current-table)
                        (user-error "No PGmacs table in current buffer")))
         (current-row (pgmacstbl-current-object))
         (cols (pgmacstbl-columns pgmacstbl))
...
Deletion is only possible when the table has primary key."
  ...
  (when (null pgmacs--table-primary-keys)
    (user-error "Can't edit content of a table that has no PRIMARY KEY"))
  (let* ((pgmacstbl (or (pgmacstbl-current-table)
                        (user-error "No PGmacs table at point")))
         (cols (pgmacstbl-columns pgmacstbl))
         (current-row (pgmacstbl-current-object))
         (where-clauses (list))
         (where-values (list))
  ...
  (let ((entries (list)))
    (dolist (table (pg-tables pgmacs--con))
      (let* ((schema (if (pg-qualified-name-p table)
             ...
             ;; The table owner is queried once the basic table information has been displayed, for
             ;; responsiveness.
             (owner nil))
...
(defun pgmacs--display-procedures/infschema (&rest _ignore)
  ...
  (let* ((db-buffer pgmacs--db-buffer)
    ...
    (let ((inhibit-read-only t))
      (erase-buffer)
      (remove-overlays)
      (insert (propertize "PostgreSQL functions and procedures" 'face 'bold 'display '(height 1.3)))
      (insert "\n\n")
...
(cl-defun pgmacs--row-list-unmark-all (&rest _ignore)
  ...
  (interactive)
  (let* ((table (or (pgmacstbl-current-table)
                    (user-error "No PGmacs table at point")))
         (buffer-read-only nil))
  ...
  (let* ((con pgmacs--con)
    ...
    (let* ((offset (or pgmacs--offset 0))
           ...
           (columns (cl-loop
                     ...
                     for dpy = (or (pgmacs--lookup-column-displayer table name)
                                   (pgmacs--make-column-displayer "" (make-hash-table)))
